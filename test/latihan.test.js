...
test('greeting component renders {nama}', () => {
    render(<Greeting name="Zahrap" />);
    const greetingMessage = screen.getByTestId('greeting');
    expect(greetingMessage).toHaveTextContent('Hello, Zahrap');
});


test('triggers alert with correct message when clicked', () => {
    window.alert = jest.fn(); 
    render(<AlertButton message="Hello, Zahrap!" />);
    const button = screen.getByTestId('alert-button');
    fireEvent.click(button);
    expect(window.alert).toHaveBeenCalledWith('Hello, Zahrap!');
});
